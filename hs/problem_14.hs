...
getNext :: Int -> Int
getNext n = if even n then div n 2 else 3 * n + 1
...
cachedFindMaxLength' i limit maxN maxLen cache =
  if i <= limit
    then cachedFindMaxLength' (i + 1) limit maxN' maxLen' cache'
    else (maxLen, maxN)
  where
    (len, cache') = cachedCollatzLength i cache
